...
function count_users($cnx){
	...
	$result = pg_query($cnx, "SELECT count(*) FROM usuarios where nomusu<>'admin'"); 
...
function get_users_paging($cnx,$page,$qtdPorPagina) {
	...
	$sql = "SELECT * from usuarios where nomusu<>'admin' ORDER  BY nomusu LIMIT  $qtdPorPagina offset $offset";
...
function get_users($cnx) {
	$result = pg_query($cnx, "SELECT * FROM usuarios");
	if (!$result) {
	    echo "Um Erro Aconteceu\n";
	    exit;
	}
