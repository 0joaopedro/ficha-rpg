...
//===================================================
   class banco{
      ...
      function __construct() {
         $this->cnx=pg_connect("host=hard.uniguacu.edu.br dbname=2018103696 user=2018103696 password=3696") or die ("Nao conectou");
      } 
      function consulta($sql){
         $this->res=@pg_query($sql); 
      }
      function lista2($name='tabela1',$inc='',$alt='',$exc='',$img='') {
         if($inc) echo("<p align=\"center\"><a class=\"btn btn-info\" href=\"$inc\">Incluir</a></p>");
         ...
         while($line=@pg_fetch_object($this->res)){
            ...
            if($alt or $exc or $img){
               ...
               if($alt) echo("<a style=\"width:80px\" class=\"btn btn-info\" href=\"$alt?alt=".$line->$codigo."\">Alterar</a> ");
               if($exc) echo("<a style=\"width:80px\" class=\"btn btn-info\" href=\"$exc?exc=".$line->$codigo."\">Excluir</a> ");
               if($img) echo("<a style=\"width:80px\" class=\"btn btn-info\" href=\"$img?cod=".$line->$codigo."\">Imgs</a> ");
               echo("</td>");
            }   
            echo("</tr>");
         }        
         ...
         echo("  <script> $(document).ready(function() { $('#$name').DataTable({
            'language': {'url': '../include/DT/Portuguese-Brasil.json'},
  ...
  <!DOCTYPE html>
   ...
   function seguranca(){
      if(isset($_GET))$_GET=str_replace("'","`",$_GET);
      if(isset($_POST))$_POST=str_replace("'","`",$_POST);
   }
   ...
   class form{
      ...
      function show($name='frm',$botoes=''){
         ...
         if($botoes){
            echo("<div align=\"center\">$botoes</div>");
         }else{
            echo("<div align=\"center\"><input style=\"width:200px\" class=\"btn btn-warning\"  type=\"button\" value=\"CANCELAR\" onclick=\"history.go(-1)\"><input style=\"width:200px\" class=\"btn btn-info\"  type=\"button\" value=\"SALVAR\" onclick=\"valida$name()\"></div>\n");
         }   
         echo("</form>\n</div>\n");
      }
   }
